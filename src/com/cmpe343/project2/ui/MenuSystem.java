...
public class MenuSystem {
    ...
    private void printContacts(List<Contact> list) {
        ...
        String contactFormat = "%-4s │ %-12s │ %-12s │ %-12s │ %-12s │ %-14s │ %-14s │ %-25s │ %-12s │ %-20s │ %-20s │ %-18s";
        String header = String.format(contactFormat,
                "ID", "First", "Middle", "Last", "Nickname", "Phone (P)", "Phone (S)", "Email",
                "Birth", "Created", "Updated", "LinkedIn");

        printBorderedHeader(header, "Contacts Directory");

        int rowIndex = 0;
        for (Contact c : list) {
            String row = String.format(contactFormat,
                    c.getContactId(),
                    safe(c.getFirstName()),
            ...
            String rowColor = rowIndex % 2 == 0 ? ConsoleColors.WHITE_BOLD : ConsoleColors.GREEN;
            System.out.println(ConsoleColors.CYAN + "║ " + rowColor + row + ConsoleColors.CYAN + " ║" + ConsoleColors.RESET);
            if (rowIndex < list.size() - 1) {
                printRowSeparator(header.length());
            }
            rowIndex++;
        }

        printBorderLine(header.length(), "╚", "╝");
    }
    ...
    private void printBorderedHeader(String header, String title) {
        printTableTitle(title, header.length());
        printBorderLine(header.length(), "╔", "╗");
        System.out.println(ConsoleColors.CYAN + "║ " + ConsoleColors.YELLOW_BOLD + header + ConsoleColors.CYAN + " ║" + ConsoleColors.RESET);
        printBorderLine(header.length(), "╠", "╣");
    }

    private void printBorderLine(int headerLength, String left, String right) {
        String border = "═".repeat(headerLength + 2);
        System.out.println(ConsoleColors.CYAN + left + border + right + ConsoleColors.RESET);
    }

    private void printUsers(List<User> list) {
        String userFormat = "%-5s │ %-15s │ %-15s";
        String header = String.format(userFormat, "ID", "Username", "Role");

        printBorderedHeader(header, "User Registry");

        int rowIndex = 0;
        for (User u : list) {
            String row = String.format(userFormat, u.getUserId(), u.getUsername(), u.getRole());
            String rowColor = rowIndex % 2 == 0 ? ConsoleColors.WHITE_BOLD : ConsoleColors.GREEN;
            System.out.println(ConsoleColors.CYAN + "║ " + rowColor + row + ConsoleColors.CYAN + " ║" + ConsoleColors.RESET);
            if (rowIndex < list.size() - 1) {
                printRowSeparator(header.length());
    ...
    private void printRowSeparator(int headerLength) {
        String divider = "─".repeat(headerLength + 2);
        System.out.println(ConsoleColors.CYAN + "╟" + divider + "╢" + ConsoleColors.RESET);
    }

    private void printTableTitle(String title, int headerLength) {
        String paddedTitle = " " + title.toUpperCase() + " ";
        int interiorWidth = headerLength + 2;
        int remaining = Math.max(0, interiorWidth - paddedTitle.length());
        int left = remaining / 2;
        int right = remaining - left;
        System.out.println(ConsoleColors.PURPLE_BOLD + "╔" + "═".repeat(left)
                + ConsoleColors.YELLOW_BOLD + paddedTitle
                + ConsoleColors.PURPLE_BOLD + "═".repeat(right) + "╗" + ConsoleColors.RESET);
    }
